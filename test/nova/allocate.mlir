...
func.func @allocate_space_assignment(%lhs_src: memref<16x32xf32>, %rhs_src: memref<32x8xf32>, %scale_src: memref<16x8xf32>, %bias_src: memref<16x8xf32>) {
  %lhs = nova.load(%lhs_src) {start = [0, 0]} : memref<16x32xf32> -> tensor<16x32xf32>
  %rhs = nova.load(%rhs_src) {start = [0, 0]} : memref<32x8xf32> -> tensor<32x8xf32>
...
// CHECK: %[[LOAD0:.*]] = nova.load(%arg0) {start = [0, 0]} : memref<64x16xf32> -> tensor<16x16xf32, {bank0 = 0 : i64, space = 3 : i64}>
// CHECK: %[[SQUARE0:.*]] = nova.square(%[[LOAD0]]) : tensor<16x16xf32, {bank0 = 0 : i64, space = 3 : i64}> -> tensor<16x16xf32, {bank0 = 1 : i64, space = 3 : i64}>
// CHECK: nova.store(%[[SQUARE0]], %arg1) {start = [0, 0]} : (tensor<16x16xf32, {bank0 = 1 : i64, space = 3 : i64}>, memref<64x16xf32>) -> ()
// CHECK: %[[LOAD1:.*]] = nova.load(%arg0) {start = [1, 0]} : memref<64x16xf32> -> tensor<16x16xf32, {bank0 = 0 : i64, space = 3 : i64}>
...
// CHECK: %[[RHS:.*]] = nova.load(%arg1) {start = [0, 0]} : memref<32x8xf32> -> tensor<32x8xf32, {bank0 = 1 : i64, space = 3 : i64}>
// CHECK: %[[SCALE:.*]] = nova.load(%arg2) {start = [0, 0]} : memref<16x8xf32> -> tensor<16x8xf32, {bank0 = 0 : i64, space = 4 : i64}>
// CHECK: %[[BIAS:.*]] = nova.load(%arg3) {start = [0, 0]} : memref<16x8xf32> -> tensor<16x8xf32, {bank0 = 0 : i64, space = 5 : i64}>
