...
func.func @allocate_space_assignment(%lhs_src: memref<16x32xf32>, %rhs_src: memref<32x8xf32>, %scale_src: memref<16x8xf32>, %bias_src: memref<16x8xf32>) {
  %lhs = nova.load(%lhs_src) {index = array<i64: 0, 0>} : memref<16x32xf32> -> tensor<16x32xf32>
  %rhs = nova.load(%rhs_src) {index = array<i64: 0, 0>} : memref<32x8xf32> -> tensor<32x8xf32>
  %scale = nova.load(%scale_src) {index = array<i64: 0, 0>} : memref<16x8xf32> -> tensor<16x8xf32>
  %bias = nova.load(%bias_src) {index = array<i64: 0, 0>} : memref<16x8xf32> -> tensor<16x8xf32>
...
// CHECK: %[[LOAD0:.*]] = nova.load(%arg0) {index = array<i64: 0, 0>} : memref<64x16xf32> -> tensor<16x16xf32, {bank = 0 : i64, space = 3 : i64}>
// CHECK: %[[SQUARE0:.*]] = nova.square(%[[LOAD0]]) : tensor<16x16xf32, {bank = 0 : i64, space = 3 : i64}> -> tensor<16x16xf32, {bank = 2 : i64, space = 3 : i64}>
// CHECK: nova.store(%[[SQUARE0]], %arg1) {index = array<i64: 0, 0>} : (tensor<16x16xf32, {bank = 2 : i64, space = 3 : i64}>, memref<64x16xf32>) -> ()
// CHECK: %[[LOAD1:.*]] = nova.load(%arg0) {index = array<i64: 1, 0>} : memref<64x16xf32> -> tensor<16x16xf32, {bank = 0 : i64, space = 3 : i64}>
...
// CHECK: nova.store(%[[SQUARE1]], %arg1) {index = array<i64: 1, 0>} : (tensor<16x16xf32, {bank = 2 : i64, space = 3 : i64}>, memref<64x16xf32>) -> ()
...
// CHECK: nova.store(%[[SMALL]], %arg2) {index = array<i64: 0, 0>} : (tensor<16x16xf32, {bank = 4 : i64, space = 3 : i64}>, memref<16x16xf32>) -> ()
// CHECK: nova.free(%[[LARGE]]) : tensor<70000xf32, {bank = 0 : i64, space = 3 : i64}>
// CHECK-LABEL: func.func @allocate_space_assignment
// CHECK: %[[LHS:.*]] = nova.load(%arg0) {index = array<i64: 0, 0>} : memref<16x32xf32> -> tensor<16x32xf32, {bank = 0 : i64, space = 3 : i64}>
// CHECK: %[[RHS:.*]] = nova.load(%arg1) {index = array<i64: 0, 0>} : memref<32x8xf32> -> tensor<32x8xf32, {bank = 2 : i64, space = 3 : i64}>
// CHECK: %[[SCALE:.*]] = nova.load(%arg2) {index = array<i64: 0, 0>} : memref<16x8xf32> -> tensor<16x8xf32, {space = 4 : i64}>
// CHECK: %[[BIAS:.*]] = nova.load(%arg3) {index = array<i64: 0, 0>} : memref<16x8xf32> -> tensor<16x8xf32, {space = 5 : i64}>
